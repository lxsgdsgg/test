<template>
  ...
  >
        ...
        <div>
          <div style="margin-top: 10px;">
            <div style="background-color: #EEEEEE;line-height: 30px;">
              <span>中介类分成信息
                <el-button type="primary" @click="sureDividInfos('01','0')" size="mini"
                           v-if="resultData.dealInfo.agentDividCnt === 0 && resultData.dealInfo.isBreach !== 1"
                           class="pull-right" :loading="confirmDivideBtn">确认分成</el-button>
                <el-button type="primary" @click="handlerModifyDivides('0')"
                           v-if="resultData.dealInfo.agentDividCnt === 0 && resultData.dealInfo.isBreach !== 1"
                           size="mini" class="pull-right mr10">修改分成</el-button>
              </span>
            </div>
          </div>
            ...
            <div style="background-color: #EEEEEE;line-height: 30px;">
              <span>金融类分成信息
                  <el-button v-hasMultipleBtn="['confirmFinDivide',resultData.dealInfo]" type='primary'
                     v-if="resultData.dealInfo.FinceDividCnt === 0 && resultData.dealInfo.isBreach !== 1"
                             @click="sureDividInfos('01','1')" size="mini" class="pull-right" :loading="confirmDivideBtn">确认分成</el-button>
    ...
    <template>
      ...
      >
        <add-follow @submit="handleFollowSubmit" :peopleSelectOpts="peopleSelectOpts" :dealId="params.dealId" ref="follow" :dealType="'01'"></add-follow>
      </el-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      isShowPhoneList(show){
        ...
        if(show){
          this.dealAddLog(this.dealId,this.dealId,'7','431','查看电话')
        }
      },
      ...
      toDemandDetail(id,houseUsed){
        this.$router.push({ path: '/customer/customerDetail/' + id
          ,  query: {formName: houseUsed + '_customer', transactionTypeId: 2}})
        // this.$router.push({ path: '/customer/customerDetail', query: { demandId: id
        //     , formName: houseUsed || '_customer',transactionTypeId: 2}})//transactionTypeId = 2 （出租）
      },
      // 进入房源详细
      ...
      sureDividInfos(dealType,divideType){
        ...
        this.$confirm('确认分成后无法撤销，要提交分成确认吗？','重要警告',{
          ...
          RequestURL.sureDividInfos(params).then(res=>{
            ...
            this.dealQueryLog(this.dealId,this.dealId,'7','411','确认分成')
          }).catch(err=>{
            this.confirmDivideBtn = false
            this.$message({
              type: 'error',
      ...
      dealBreach(){
        this.$confirm('确认要提交违约吗？','警告',{
          ...
          RequestURL.dealBreach(params).then(res=>{
            ...
            this._getTableData()
            this.dealQueryLog(this.dealId,this.dealId,'7','412','违约')
            this.$emit('submit',1)
          }).catch(err=>{
            this.dealBreachBtn = false
            this.$message({
      ...
      cancelDealBreach(){
        this.$confirm('确认要取消违约吗？','警告',{
          ...
          RequestURL.cancelDealBreach(params).then(res=>{
            ...
            this._getTableData()
            this.dealQueryLog(this.dealId,this.dealId,'7','413','取消违约')
            this.$emit('submit',0)
          }).catch(err=>{
            this.cancelDealBreachBtn = false
            this.$message({
      ...
      _getTableData(){
        let params = {id: this.params.dealId,dealType: '01'}
        RequestURL.queryDealInfoByIdAndType(params).then(res=>{
          if(res.price){
            res.price = (res.price/100) + '元'
          }

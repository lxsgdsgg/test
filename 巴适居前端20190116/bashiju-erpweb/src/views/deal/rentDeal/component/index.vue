    ...
    <div class="wrapper" v-if="dealInfo">
      <el-tabs v-model="activeName" @tab-click="handleTabClick" type="card" style="max-width: 1280px">
        <el-tab-pane label="基本信息" name="dealInfo">
          <div>
            ...
            <div class="pull-right" style="margin-top: 1px;">
              <el-button v-hasMultipleBtn="['updateDealInfo',dealInfo]" type="primary"
                         size="mini" @click="handlerModify" v-if="dealInfo.isBreach !== 1">修改成交</el-button>
          ...
          <base-info
            ...
            ref="dealInfo"
            :dealType="'01'"
            :dealInfo="dealInfo" @submit="handlerReset"
            :peopleSelectOpts="peopleSelectOpts">
          </base-info>
        </el-tab-pane>

        <el-tab-pane label="佣金记录" name="commissionRecord">
          <commission-record ref="commissionRecord" :dealId="dealId"
                             :dealInfo="dealInfo" :dealType="'01'"></commission-record>
        </el-tab-pane>
        ...
        <el-tab-pane label="文件扫描件" name="scanDealFile">
          ...
          <hr>
          <photo-upload :defaultParams="{dealId: dealId, attachType: attachType,dealType: '01'}"
                        :dealType="'01'" :dealInfo="dealInfo"ref="scanDealFile"></photo-upload>
        </el-tab-pane>

        <el-tab-pane label="跟进记录" name="followRecord">
          <div class="pull-right" style="margin-top: 1px;">
            <el-button v-hasMultipleBtn="['addFollowRecord',dealInfo]"
                       type="primary" size="mini" @click="handleOpenDialogFollow" v-if="dealInfo.isBreach !== 1">新增</el-button>
          </div>
          <follow-record ref="followRecord" :dealId="dealId" :dealType="'01'"></follow-record>
        </el-tab-pane>
        ...
        <el-tab-pane label="代收付款" name="paymentToBeReceived">
          <payment-to-be-received ref="paymentToBeReceived"
                                  :dealId="dealId" :dealInfo="dealInfo" :dealType="'01'"></payment-to-be-received>
        </el-tab-pane>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 合同打印
      printContract(){
        this.dealQueryLog(this.dealId, this.dealId,'7', '410', '合同打印')
      ...
      _getTableData(){
        if(this.dealId === undefined){
          ...
          let params = {id: this.dealId,dealType: '01'}

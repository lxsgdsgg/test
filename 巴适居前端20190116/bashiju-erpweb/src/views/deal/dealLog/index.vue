    ...
    <div class="page-content-hd">
      <el-form :model="queryForm" ref="queryForm" :inline="true" size="small" v-if="operatorTypeObj">
        <el-form-item prop="actionType" label="操作类型">
          <el-select v-model="queryForm.actionType" style="width: 100px" filterable>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toDetail (row) {
        this.$router.push({ path: './dealDetail/' + row.sourceId})
      },
      _loadData(btnQuery) {
        if (btnQuery) {
      ...
      _getTableData() {
        ...
        RequestURL.queryDealActionLogPageList(params).then(res => {

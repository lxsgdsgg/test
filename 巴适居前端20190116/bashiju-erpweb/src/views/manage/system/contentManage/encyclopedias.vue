      ...
      <div class="query-form">
        <el-form size="small" ref="queryForm" :inline="true" :model="queryForm" class="demo-form-inline">
          ...
          <el-form-item class="pull-right">
            <el-button v-hasOnlyBtn="'encyclopediasAddBtn'" type="primary" @click="handleAdd">添加文章</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button  v-hasMultipleBtn="['encyclopediasShowBtn',scope.row]"  @click="handleShow(scope.row)" type="text" size="small">查看</el-button>
            <el-button  v-hasMultipleBtn="['encyclopediasUpdateBtn',scope.row]" v-if="scope.row.examineStatus==null || scope.row.examineStatus==0" @click="handleEdit(scope.row)" type="text" size="small">编辑</el-button>
            <el-button v-hasMultipleBtn="['encyclopediasUpdateBtn',scope.row]" v-if="scope.row.isSetTop==0"  @click="handleSetTop1(scope.row)" type="text" size="small">置顶</el-button>
            <el-button v-hasMultipleBtn="['encyclopediasUpdateBtn',scope.row]" v-if="scope.row.isSetTop==1"  @click="handleSetTop2(scope.row)" type="text" size="small">取消置顶</el-button>
            <el-button v-hasMultipleBtn="['encyclopediasUpdateBtn',scope.row]" v-if="scope.row.isPublished==0"  @click="handleSetPublished1(scope.row)" type="text" size="small">发布</el-button>
            <el-button v-hasMultipleBtn="['encyclopediasUpdateBtn',scope.row]" v-if="scope.row.isPublished==1"  @click="handleSetPublished2(scope.row)" type="text" size="small">撤回</el-button>
            <el-button  v-hasMultipleBtn="['encyclopediasDelBtn',scope.row]"  @click="handleDelete(scope.row)" type="text" size="small">删除</el-button>
          </template>
        </el-table-column>
          ...
          <el-form v-loading="loadingForm" :model="addForm" status-icon :rules="addFormRules" ref="addForm" label-width="100px">
            ...
            <el-form-item label="文章分类" prop="classesId">
              <classType-selector
                ...
                :url="articleUrl"
                :props="cascaderProps"
                :dataProps="cascaderDataProps" @change="changeType">
              </classType-selector>
            </el-form-item>
...
<script>
  ...
  export default {
    ...
    mixins: [PageList],
    components: {BaseCityCascader,classTypeSelector, BaseEditor},
    ...
    methods: {
      ...
      changeCity(){
        ...
        this.articleUrl= 'manage/getEncyclopediasClasse?cityCode='+cityCode;
        ...
        this.$refs['addForm'].validate((valid) => {
          if (valid) {
           ...
           // alert(this.$refs['editor'].getContent());
            ...
            this.addForm.type = 'encyclopedias';
            const params = {
            ...
            this.$confirm('确定保存编辑的信息吗？, 是否继续?', {
              ...
              RequestURL.saveArticle(params).then(res => {
                ...
                if (res.success) {
                  ...
                  //记日志
                  if(this.isAdd){//新增
                      let logContent = '新增了一篇百科，分类：'+this.classesName+";标题："+this.addForm.title
                      let message = {
                          sourceCode: '房产百科',
                          sourceTypeId: 26,
                          operatTypeId: 1,
      ...
      handleDelete (row) {
        this.$confirm('确定删除该条数据？, 是否继续?', {
          ...
          RequestURL.delArticle({id: row.id,type:'0'}).then(res => {
            ...
            //写日志
            let logContent = '删除了一篇百科，分类：'+row.typeName+";标题："+row.title
            let message = {
                sourceCode: '房产百科',
                sourceTypeId: 26,
                operatTypeId: 3,
      ...
      _loadData (btnQuery) {
        ...
        this.nowQueryForm.type = 'encyclopedias';
      ...
      _setForm () {
        ...
        this.$utils.setFormInfo(this.addForm, this.currentRowData, () => {
          ...
          this.articleUrl= 'manage/getEncyclopediasClasse?cityCode='+ this.currentRowData['cityCode']

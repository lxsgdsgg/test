            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <el-button v-hasOnlyBtn="'userDynamicFormAddBtn'" @click="handleAdd(scope.row)" type="text" size="small">新增字段</el-button>
                </template>
              </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      selectHasField(){
        RequestURL.queryCompanyDynamicFormFieldSelect({formId:this.currentDynamicConfigureItem.id}).then(res => {
          console.log(res);
          if (res.success) {
            this.selectField = [];
      ...
      handleSubmit () {
        ...
        this.$refs['addForm'].validate((valid) => {
          if (valid) {
            ...
            this.addForm.customTableName = this.customTableName
            const params = {
            ...
            this.$confirm('确定保存编辑的信息吗？, 是否继续?', {
              ...
              RequestURL.saveCompanyFormField(params).then(res => {
                ...
                if (res.success) {
                  ...
                  this._loadValueData(this.currentDynamicConfigureItem.id);
                  //记日志
                  if(this.isAdd){//新增
      ...
      handleDelete (row) {
        this.$confirm('确定删除该条数据？, 是否继续?', {
          ...
          RequestURL.delCompanyFormField({id: row.id}).then(res => {
            this.$message({
              type: 'success',
      ...
      _loadData (btnQuery) {
        ...
        RequestURL.queryCompanyDynamicFormList(params).then(res => {
          ...
          this._loadValueData(this.tableData[0].id);
          this.customTableName = this.tableData[0].customTableName
        }).catch(() => {
          this.loadingQueryBtn = false
          this.loadingView = false
        })
      ...
      _loadValueData(formId){
        this.loadingItemView = true;
        this.queryField = {
          formId: formId
        };
        const params = this._getParams(this.queryField)
        RequestURL.queryCompanyDynamicFormFieldList(params).then(res => {
      ...
      _showValue(row, event, column){
        ...
        this.currentDynamicConfigureItem = row;
        this._loadValueData(row.id);
        this.customTableName = row.customTableName
      }
    },

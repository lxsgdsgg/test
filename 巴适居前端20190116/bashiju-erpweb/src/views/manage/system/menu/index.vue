<template>
  <div class="page-content">
    ...
    <template>
      ...
      <data-combination
        v-if="combinationDialogVisible"
        :menuId="currentNodeData && currentNodeData.id"
        :menuName="currentNodeData && currentNodeData.name"
        :show.sync="combinationDialogVisible"></data-combination>
...
export default {
  ...
  methods: {
    ...
    nodeClick (node) {
      ...
      delete obj.children
      delete obj.parent
      this.originalFormData = this.$jsonUtils.deepClone(obj)
    ...
    handleDelete () {
      this.$confirm('确定删除该条数据吗?', {
        ...
        const name = this.currentNodeData.name
          ...
          this.$confirm('确定保存编辑的信息吗?', {
            ...
            saveOrUpdateMenu({jsonData: JSON.stringify(params)}).then(res => {
              ...
              if (operate === 'add') {
                ...
                let message = {
                  sourceCode: this.form.name, // 资源标识
                  sourceTypeId: this.$DICT_CODE.LOG.SOURCE_TYPE.SYSTEM.MENU, // 资源类型
                  operatTypeId: this.$DICT_CODE.LOG.OPERATE_TYPE.ADD, // 操作类型
                  logContent: `新增菜单：${this.form.name}` // 日志内容
                }

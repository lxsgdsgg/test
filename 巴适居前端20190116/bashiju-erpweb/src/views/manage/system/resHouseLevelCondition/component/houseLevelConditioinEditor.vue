              ...
              <el-row>
                   <el-col :span="12">
                     ...
                     <el-form-item label="条件" prop="conditions">
                           <base-select @change="getConditionName" style="width: 100%" :multiple="true" :props="conditionsProp" :data="conditionsOptions"  v-model="form.conditions" collapse-tags></base-select>
                        </el-form-item>
                    ...
                    <el-col :span="12">
                      ...
                      <el-form-item label="所属区域" prop="areaCode">
                          <base-cascader @change="handleChangeArea" clearable :changeOnSelect="false" v-model="form.areaCode" :data="regionSelectOpts" :props="cascaderProps" :dataProps="cascaderDataProps"></base-cascader>
                      </el-form-item>
...
<script>
  ...
  import {getDynamicValue,getDynamicValueConditions,getPriceAndSpaceOption,saveOrUpdateResHouseLevelCondition,queryAreaAndRegion,dropDownData} from '@/request/manage/resHouseLevelCondition'
    ...
    components:{BaseSelect,CitySelector,BaseSectionSelect,BaseCascader},
      ...
      data() {
        ...
        methods: {
          ...
          setForm(data){
            for(let i in  this.form){
              ...
              //处理父组件传过的值
                ...
                if(data['regionId'])this.form['regionId'] =  data['regionId'].split(',').map(Number)
              console.log(typeof data['regionId'])
              ...
              this.$refs['form'].validate(res =>{
                  if(res){
                    this.$confirm('确定要保存吗?',{
                      ...
                      params.remark ='城市:-'+this.form.cityName+  ';区域为-' + this.form.area+';片区:'+this.regionNameOpt+';条件为:'+this.conditionName+';';
                      //判断是否为新增
                      params.maxPrice = params.maxPrice *1000000
                      params.minPrice = params.minPrice*1000000
                      params.regionName = this.regionNameOpt
           ...
           */
          houseUses(){
            dropDownData().then(res =>{
               this.houseUseOpt = res.houseUses
            })
          },
          ...
          handleChangeArea (val, name, data) {
            this.form.area = name
            this.form.regionId = []
          },
          handleChangeRegion(value,name){
            this.regionNameOpt = name.join(',')
          },
          //获取普通下拉
          getDynamicValue(){
      ...
      watch: {
       ...
       'form.cityCode'(newValue) {
         ...
         selectUrl.queryAreaByCityCode({cityCode: newValue || ''}).then(res => {
           this.regionSelectOpts = res || []
         })
        },
       //监听片区选择
       'form.areaCode'(newValue) {
         ...
         selectUrl.queryRegionByAreaCode({areaCode: newValue || ''}).then(res => {
           this.dateOpt = res || []
         })
        }
      }

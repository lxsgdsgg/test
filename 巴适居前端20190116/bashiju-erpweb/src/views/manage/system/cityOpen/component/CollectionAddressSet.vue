  ...
  <div class="page-content">
    <div>
      <el-form size="small" :inline="true" :model="form" ref="form">
        ...
        <el-form-item>
          <el-button  type="primary" @click.native.prevent="_loadData(false)" :loading="loadingBtn">查询</el-button>
          <el-button type="info"  @click.native.prevent="handleReset">清空</el-button>
    ...
    <template>
      ...
      >
        <address-config-editor-set :cityCode="data.cityCode" :data="currentRowData" v-if="hackReset" :isAdd="isAdd" @handleClick="openOrCloseDialog"></address-config-editor-set>
      </el-dialog>
    </template>
  </div>
</template>
...
<script>
  ...
  import AddressConfigEditorSet from'./addressConfigEditor'
  import  *  as  RequestUrl from '@/request/log/systemPlatformLog'
  export default {
    name:'CollectionAddressSet',
    mixins:[PageList],
    components:{BaseSelect,CitySelector,AddressConfigEditorSet},
    props:{
      data: {
        type: Object,
        required: true,

         ...
         // this.loadingSureBtn = true
          ...
          RequestURL.sureArticle(params).then(res => {
            ...
            if (res.success) {
              ...
              let operatTypeId = ''
              if(row.type==0){
                logContent = '快讯；'+"标题："+row.title+";审核通过"
              }else{
                logContent = '公告；类型：'+row.classesName+";标题："+row.title+";审核通过"
              }
              if(row.type==0){
                operatTypeId = 608
              }else{
                 ...
                 this.$confirm('要驳回吗？, 是否继续?', {
                  ...
                  RequestURL.rejectArticle(params).then(res => {
                    ...
                    if(this.typeForReject==0){
                      logContent = '快讯；'+"标题："+this.titleForReject+";被驳回"
                    }else{
                      logContent = '公告；类型：'+this.classesNameForReject+";标题："+this.titleForReject+";被驳回"
                    }
                    if(row.type==0){
                      operatTypeId = 609
                    }else{
                    ...
                    let message = {
                        ...
                        sourceCode: this.titleForReject,
                        businessTypeId: 6,//业务类型，
                        sourceTypeId: 14,
                        operatTypeId: operatTypeId,
                        logContent: logContent,

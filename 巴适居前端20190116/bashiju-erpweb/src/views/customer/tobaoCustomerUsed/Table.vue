      ...
      <template v-for="(fruit, index) in _formThead">
        ...
        <el-table-column v-else :key="index" :label="fruit.label && fruit.label" :align="fruit.align && fruit.align" :width="fruit.width && fruit.width"    show-overflow-tooltip>
          <template slot-scope="scope">
            ...
            <a
              @click="toDetail(scope.row.id, scope.row.formName)"
            ...
            <span v-else-if="fruit.prop === 'room'">
              {{ scope.row.minRoom == 0 ? scope.row.minRoom+'室以上' : scope.row.minRoom+'-'+scope.row.maxRoom+'室' ||  scope.row.maxRoom == 0 ? scope.row.maxRoom +'室以下' : scope.row.minRoom+'-'+scope.row.maxRoom+'室'  }}
            </span>


            <span v-else-if="fruit.prop === 'price'">
              {{ scope.row.minPrice + '-' + scope.row.maxPrice+'以上' }}
            </span>


            <span v-else-if="fruit.prop === 'space'">
              {{ scope.row.minSpace + '-' + scope.row.maxSpace+'以上' }}
            </span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toDetail (id, name) {
        this.$router.push({ path: '/customer/taobaoCustomerDetail', query: { demandId: id, formName: name}})

    ...
    <div class="wrapper">
      ...
      <template>
        ...
        <el-dialog
          ...
          :close-on-press-escape="false"
        >
          <write-look-house @submit="handleLookHouseSubmit" :topParams="topParams" v-if="hackReset"
                            :regionValue="regionValue" :deptSelectOpts="peopleSelectOpts" ref="writelook"></write-look-house>
        </el-dialog>
    ...
    methods: {
     ...
     handleChangeHouse () {
      ...
      openDialogVisibleUpdateStatus(){
        ...
        if(this.customerInfo.statusId === '1'
          || this.customerInfo.statusId === '7'
          || this.customerInfo.statusId === '8'){
          this.statusOpts = consts.FOUR_OPTS
        }else if(this.customerInfo.statusId === '3'
          || this.customerInfo.statusId === '4'
          || this.customerInfo.statusId === '5'){
          this.statusOpts = consts.TWO_OPTS
        }
        ...
        this.$refs['formUpdateStatus'].validate((valid) => {
          if (valid) {
            this.$confirm('确定保存编辑的信息吗？', {
              ...
              let params = {
                examineInfo: {
                  content: `房源状态由[${this.customerInfo.status}]变更为[${statusName}],原因是：${this.formUpdateStatus.content}`,
                  remark: {

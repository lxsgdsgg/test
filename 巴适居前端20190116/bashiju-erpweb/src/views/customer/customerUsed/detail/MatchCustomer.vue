  ...
  <div style="min-height: 550px">
    <div >
      <el-form ref="queryForm" :inline="true" size="small">
        <el-form-item label="" prop="selectId" ref="communityId" >
              <community-selector
                style="width: 150px"
    ...
    methods: {
      ...
      _getTableData (btn) {
        ...
        if(this.transactionTypeId==4){
          minSellingPrice = this.queryForm.price && this.queryForm.price.min || ''  //出售
          maxSellingPrice = this.queryForm.price && this.queryForm.price.max || ''
          delete this.queryForm.price
        }else{
          minRentPrice = this.queryForm.price && this.queryForm.price.min || '' //出租
          maxRentPrice = this.queryForm.price && this.queryForm.price.max || ''
          delete this.queryForm.price
        }
        if(!this.queryForm.selectId){
        ...
        matchingHousing(params).then(res => {
          this.tableData = res.data.map(item => {
                 if(item.transactionTypeId==2){
                    item.price = item.rentPrice+'元'
                 }else{
                   item.price = item.sellingPrice+'万元'
      ...
      handleCommunityChange(val, name, data){
          ...
          this.queryForm.selectId = data.code

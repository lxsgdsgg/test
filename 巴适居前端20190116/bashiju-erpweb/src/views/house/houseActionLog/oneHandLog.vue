  ...
  <div class="page-content">
    ...
    <div class="page-content-hd">
      <el-table :data="tableData" border align="center" tooltip-effect="light" size="small" v-loading="loadingView">
        ...
        <el-table-column label="项目标题" prop="sourceCode" show-overflow-tooltip width=140px>
          <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      toDetail (row) {
        this.$router.push({ path: '/house/houseNewDetail/'
            + row.sourceId + '/'
            + row.sourceCode, query: {menu_id: '83'}})//menuId用做权限查询，这里获取不到先写固定
      },
      ...
      _getTableData() {
        ...
        RequestURL.auxOneHandHouseActionLogPageList(params).then(res => {
      ...
      returnOperatorTypeObj(){
        RequestURL.returnOperatorTypeObj({type: '3'}).then(res=>{

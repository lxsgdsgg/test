  ...
  export default {
    ...
    methods: {
            ...
            if (valid) {
              ...
              this.$confirm('确定保存编辑的信息吗?', {
                ...
                updateOwnerPhone({...this.formPhone}, cfg).then(res => {
                  ...
                  this.$emit('handleBtnClick', 1, 2, {...this.formPhone}, false)
              ...
              if (valid) {
                ...
                this.$confirm('确定保存编辑的信息吗?', {
                  ...
                  saveOwnerPhone({...this.formPhone}, cfg).then(res => {
                    ...
                    this.$emit('handleBtnClick', 1, 2, {...this.formPhone}, true)

                    debugger
                    // 向后台传递日志数据
                    let message = Object.assign({}, this.logMessage, {
                      operatTypeId: this.$DICT_CODE.LOG.BUSINESS_OPERATE_TYPE.ADD_PHONE, // 操作类型
                      logContent: `电话：${this.formPhone.phone}, 备注：${this.formPhone.remark}`
                    })
      ...
      _setForm () {
        ...
        if (this.type === 1) {
          this.originalBaseFormData = this.$jsonUtils.deepClone(this.data)
          this.formBase = this.data
        } else if (this.type === 2) {
          this.originalPhoneFormData = this.$jsonUtils.deepClone(this.data)

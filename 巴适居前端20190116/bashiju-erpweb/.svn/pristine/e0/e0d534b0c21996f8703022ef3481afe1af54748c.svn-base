      ...
      <el-col :span="8" class="houseInfo-right">
        ...
        <el-card class="agent-info">
          <div slot="header" class="clearfix">
            <span>相关经纪人</span>
            <span class="pull-right text-red">*红色为预期业绩（元）</span>
          </div>
    ...
    methods: {
      ...
      handleShowPhoneList () {
        ...
        this._queryHousePrivateInfo(2).then((res) => {
          ...
          // 向后台传递日志数据
          let message = Object.assign({}, this.logMessage, {
            operatTypeId: this.$DICT_CODE.LOG.BUSINESS_OPERATE_TYPE.LOOK_HOUSE_PHONE // 操作类型
          })
          this.$updateLog.house.houseQueryLog({message: JSON.stringify(message)})
        })
      },
      ...
      // data 返回的回显数据
      handleOwnerSubmit (action, type, data, isAdd) {
        if (action === 1 && type === 1) {
        ...
        if (action === 1 && type === 2 && !isAdd) {
          ...
          this.$set(this.phoneList[$index], 'phone', data.phone)
          this.$set(this.phoneList[$index], 'remark', data.remark)
        }
      ...
      handleSaveSetOverallView () {
        this.$refs['formSetOverallView'].validate((valid) => {

      ...
      <b-dialog :show.sync="dialogShowWelfarePay" title="员工福利明细" width="750px" height="600px">
        ...
        >
          ...
          <el-table-column
            ...
            label="金额">
              <template slot-scope="scope">
                <el-input v-if="scope.row.edit" class="edit-input" size="mini" v-model="scope.row.companyPay" @blur="blurCompanyPay(scope.row)"></el-input>
  ...
  export default {
    ...
    methods: {
      ...
      blurCompanyPay(row){
        ...
        //得到第一个字符是否为负号
        var obj = row.companyPay.charAt(0);
        //先把非数字的都替换掉，除了数字和.
        obj.value = obj.value.replace(/[^\d\.]/g,'')
        //必须保证第一个为数字而不是.
        obj.value = row.value.replace(/^\./g,'')
        //保证只有出现一个.而没有多个.
        obj.value = row.value.replace(/\.{2,}/g,'.')
        //保证.只出现一次，而不能出现两次以上
        obj.value = row.value.replace('.','$#$').replace(/\./g,'').replace('$#$','.')
        //如果第一位是负号，则允许添加
        if(t == '-') {
          obj.value = '-' + obj.value
        }

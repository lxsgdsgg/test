...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleOpen(row){
          this.setOpen("确定开通吗？",1,row.id,row.companyName,row.cityName);
      },
      handleClose(row){
          this.setOpen("确定关闭吗？",3,row.id,row.companyName,row.cityName);
      },
      setOpen(tip,isOpen,id,companyName,cityName){
          let param = {
            id: id,
            isOpen: isOpen
          }
          ...
          this.$confirm(tip, {
              ...
              RequestURL.saveOrUpdateCompanyCityOpen(params).then(res => {
                ...
                if (res.success) {
                  ...
                  let logContent = statusText+companyName+'公司，对城市'+cityName+"的开通"
                      let message = {
                          sourceCode: '公司开通城市',
                          sourceTypeId: 22,
                          operatTypeId: 5,
      ...
      handleDel (row) {
        this.$confirm('确定删除该条数据？, 是否继续?', {
          ...
          RequestURL.cancelCompanyCityOpen({id: row.id}).then(res => {
            ...
            //写日志
            let logContent = '删除'+row.companyName+'公司，开通城市：'+row.cityName
            let message = {
                sourceCode: '公司开通城市',
                sourceTypeId: 22,
                operatTypeId: 3,
            ...
            this.$confirm('确定保存编辑的信息吗？, 是否继续?', {
              ...
              RequestURL.saveOrUpdateCompanyCityOpen(params).then(res => {
                ...
                if (res.success) {
                  ...
                  //记日志
                  if(this.isAdd){//新增
                      let logContent = this.addForm.companyName+'公司，开通城市：'+this.addForm.cityName+";开始时间："+this.addForm.beginTime+";截止时间："+this.addForm.endTime
                      let message = {
                          sourceCode: '公司开通城市',
                          sourceTypeId: 22,
                          operatTypeId: 1,

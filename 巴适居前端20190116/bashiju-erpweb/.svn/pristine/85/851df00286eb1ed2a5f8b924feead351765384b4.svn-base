<template>
  ...
  >
    <el-row :gutter="15" v-if="resultData">
      <el-col :span="16" class="houseInfo-left">
        <el-card class="box-card info">
          <span style="color: #99a9bf">过户进度</span>
          <div class="content clearfix">
            <el-steps :active="caculateActive" align-center finish-status="success">
              <el-step
                v-for="item in transitData"
                :title="item.estimateFinishTime"
                :description="item.scheduleName"
              >
              </el-step>
            </el-steps>
          </div>
          ...
          <div class="content clearfix">
            <template v-for="(value, key, index) in DETAIL_MAP">
              <template v-if>
                ...
                <el-col class="content-item" v-else-if="index % 2 === 0 && key == 'supplementInfo'" :span="12">
                  <div class="label">{{value}}</div>
                  <div class="text">{{resultData.dealInfo[key]}}</div>
                </el-col>
                ...
                <el-col class="content-item" v-else :span="12">
                  ...
                  <div class="text" v-else>{{resultData.dealInfo[key]}}</div>
                  <!--| formatInfo(key, resultData.dealInfo)-->
                </el-col>
              </template>
            </template>
          </div>
        ...
        <div>
          <div style="margin-top: 10px;">
            <div style="background-color: #EEEEEE;line-height: 30px;">
              <span>中介类分成信息
                <el-button type="primary" @click="sureDividInfos('00','0')" size="mini" class="pull-right"
                           :loading="confirmDivideBtn" v-if="resultData.dealInfo.agentDividCnt === 0 && resultData.dealInfo.isBreach !== 1">确认分成</el-button>
                <el-button type="primary" @click="handlerModifyDivides('0')" size="mini" class="pull-right mr10"
                           v-if="resultData.dealInfo.agentDividCnt === 0 && resultData.dealInfo.isBreach !== 1">修改分成</el-button>
              </span>
            </div>
          </div>
        ...
        <div>
          <div style="margin-top: 10px;">
            <div style="background-color: #EEEEEE;line-height: 30px;">
              <span>金融类分成信息
                  <el-button v-hasMultipleBtn="['confirmFinDivide',resultData.dealInfo]" type='primary'
                             v-if="resultData.dealInfo.FinceDividCnt === 0 && resultData.dealInfo.isBreach !== 1 " @click="sureDividInfos('00','1')" size="mini" class="pull-right" :loading="confirmDivideBtn">确认分成</el-button>
                  <el-button v-hasMultipleBtn="['updateFinDivide',resultData.dealInfo]" type='primary'
                             v-if="resultData.dealInfo.FinceDividCnt === 0 && resultData.dealInfo.isBreach !== 1" @click="handlerModifyDivides('1')" size="mini" class="pull-right mr10">修改分成</el-button>
                </span>
            </div>
          </div>
        ...
        <el-card class="box-card followRecord">
          ...
          <a href="javascript:;" @click="handleOpenDialogFollow" class="text-btn" v-if="resultData.dealInfo.isBreach !== 1">+写跟进</a>
      ...
      <el-col :span="8" class="houseInfo-right" style="margin-top: -15px;">
        ...
        <el-card class="owner-info">
          ...
          <div class="item clearfix">
            <div class="left"><span class="text-grey mr5">甲方称呼：{{contractData.partyA}}</span></div>
          </div>

          <div class="item clearfix">
            <div class="left">
              <span class="text-grey mr5">证件号码：{{contractData.cardNoA}}</span>
            </div>
          </div>
          <div class="item clearfix" style="margin-top: 10px;">
            <div class="left">
              <span class="text-grey mr5" v-if="showPhoneList">联系电话：{{contractData.mobileA}}</span>
          ...
          <hr>
          <div class="item clearfix">
            <div class="left"><span class="text-grey mr5">乙方称呼：{{contractData.partyB}}</span></div>
          </div>
          <div class="item clearfix">
            <div class="left">
        ...
        <el-button type="primary" style="width: 100%;margin-top: 5px;" v-if="!showPhoneList"
                   size="mini" @click="isShowPhoneList(true)" icon="el-icon-mobile-phone">查看电话</el-button>
        <el-button type="primary" style="width: 100%;margin-top: 5px;" icon="el-icon-mobile-phone"
            ...
            <el-table-column prop="value" label="状态" show-overflow-tooltip>
              <template slot-scope="scope">
                ...
                <div v-else-if="scope.row.isFinishCase">
                  ...
                  <div v-else>
                    <span>未结案</span>
                    <el-button class="pull-right" type="primary" size="mini" :loading="saveBtn">结案</el-button>
                  </div>
                </div>

                <div v-else-if="scope.row.transferStatus">
                  <div v-if="scope.row.transferStatus ==='00'">
                    <span>未启动</span>
                    <el-button v-hasMultipleBtn="['openTransfer',resultData.dealInfo]"
                               class="pull-right" type="primary" @click="handlerTransit()"
                               size="mini">启动过户</el-button>
                  </div>
                  <div v-else-if="scope.row.transferStatus === '01'">
                    <span style="color: #42b983">过户中</span>
                  </div>
                  <div v-else-if="scope.row.transferStatus === '02'">
                    <span style="color: #42b983">已完成</span>
                  </div>
                  <div v-else>
    ...
    <template>
      ...
      >
        <el-form :model="dividForm" ref="dividForm" :inline="true" label-width="100px" size="small">
          ...
          <div v-for="(item,index) in dividForm.editDivids">
            <el-form-item :prop="'editDivids.'+index + '.dividerId'"
            ...
            <el-form-item :prop="'editDivids.'+index +'.dividerDeptId'"
                :rules="{required: true, message: '门店不能为空', trigger: 'change'}">
              <base-dept-cascader v-model="item.dividerDeptId" style="width: 120px;"
                                  :disabled="true"></base-dept-cascader>
            </el-form-item>
            ...
            <el-form-item :prop="'editDivids.'+index +'.dividRate'" style="width: 120px;" :rules="{required: true, message: '分成比例不能为空', trigger: 'blur'}">
      ...
      <el-dialog
        title="启动过户"
        :visible.sync="dialogVisibleProc"
        width="600px"
        top="10vh"
        :close-on-click-modal="false"
      >
        <div>
          <table border="0" class="el-table">
            <tbody>
              <tr v-for="item in saveProcDetail">
                <div>
                  <td style="width: 300px">{{item.scheduleName}}</td>
                  <td>
                    <el-date-picker
          ...
          <div class="btn-group">
            <el-button type="primary" size="small" @click="saveTransferProcess" :loading="saveBtn">确定</el-button>
            <el-button type="primary" size="small" @click="startTransitHandlerCancel">取消</el-button>
          </div>
        </div>
      </el-dialog>
      ...
      <el-dialog
        title="选择过户流程"
        :visible.sync="dialogVisibleProcSel"
        width="400px"
        :close-on-click-modal="false"
      >
        <el-form v-model="selForm" ref="selForm">
          <el-form-item v-for="item in procInfos">
            <el-radio v-model="confId"
                @change="selectedRadio"
                :label="item.id"
            >
              <span style="color: #3399ff">{{item.programmeName}}</span>
            </el-radio>
          </el-form-item>
        </el-form>
      </el-dialog>
...
<script>
  ...
  import {queryContractInfo }  from '@/request/deal/houseDeal'
  ...
  export default {
    ...
    components: {BaseCascader,BaseDeptCascader,EditOwnerInfo,AddFollow},
    props: {
      params: {
        type: Object,
        required: true,
    ...
    methods: {
      ...
      isShowPhoneList(show){
        ...
        if(show){
          this.dealAddLog(this.dealId,this.dealId,'6','431','查看电话')
        }
      },
      ...
      buildStatusData(){
        ...
        temp4.name = '过户'
        temp4.value = ''
        temp4.transferStatus = this.resultData.dealInfo.transferStatus
        this.statusData.unshift(temp4)
      ...
      // 过户修改[取消]操作
      startTransitHandlerCancel(){
        this.dialogVisibleProc = false
        this.queryTransferSchedulePage()
      },

      // 取得分成配置的配置ID
      selectedRadio(confId){
        this.confId = confId
        this.getTransferProcessDetails()
      },
       ...
       */
      queryContractInfo(){
        if(!this.resultData.dealInfo.shhid){
          return false
        }
      ...
      // 根据配置ID查询过户流程详细配置信息
      getTransferProcessDetails(){
        let params = {confId: this.confId}
        RequestURL.getTransferProcessDetails(params).then(res=>{
          this.procDetail = res
          if(res){
            this.saveProcDetail = []
            for(let i in res){
              let temp = {}
              temp.scheduleName = res[i].detailName
              temp.scheduleId = res[i].id
              temp.estimateFinishTime = ''
              this.saveProcDetail.unshift(temp)
            }
          }
          this.dialogVisibleProc = true
          this.dialogVisibleProcSel = false
        }).catch(err=>{
          console.log(err)
        })
      },
      ...
      // 查询中介类分成列表
      queryTransferProcessInfo(){
        RequestURL.queryTransferProcessInfo({dealId:this.params.dealId}).then(res=>{
          this.procInfos = res
        }).catch(err=>{
          console,log(err)
        })
      ...
      handlerTransit(){
        this.chooseProcSet = true
        this.$nextTick(()=>{
          this.chooseProcSet = true
          this.confId = ''
          this.dialogVisibleProcSel = true
        })
      },

      // 启动过户
      saveTransferProcess(){
        for(let i in this.saveProcDetail){
          let estimateFinishTime = this.saveProcDetail[i].estimateFinishTime
          if(!estimateFinishTime){
            this.$message({
              type: 'warning',
              message: '预计完成时间不能为空!'
            })
        ...
        this.$confirm('确认要提交数据吗？','警告',{
          ...
          let params = {dealId: this.params.dealId,jsonData: JSON.stringify(this.saveProcDetail)}
          this.saveBtn = true
          RequestURL.saveTransferProcess(params).then(res=>{
            if(res.success){
              this.$message({
                type: 'success',
                message: res.msg
            ...
            this.dialogVisibleProc = false
            this._getTableData()
            this.dealAddLog(this.dealId,this.dealId,'6','404','启动过户')
          }).catch(err=>{
            this.saveBtn = false
            this.$message({
              type: 'error',
        ...
        this.$refs['dividForm'].validate((valid)=>{
          if(valid){
            ...
            this.$confirm('确认要提交数据吗？','警告',{
              ...
              let params = {dealId: this.params.dealId,jsonData: JSON.stringify(saveData)}
              this.saveBtn = true
        ...
        this.$refs['dividForm'].validate((valid)=>{
          if(valid){
            ...
            for(let i in this.saveDivideData){
              ...
              if(this.saveDivideData[i].id){//后台不允许两个isValid
                tempObj.isValid = this.saveDivideData[i].isValid
              }
      ...
      sureDividInfos(dealType,divideType){
        ...
        this.$confirm('确认分成后无法撤销，要提交分成确认吗？','重要警告',{
          ...
          let params = {dealId: this.params.dealId,dealType:dealType,moneyType: divideType}
          ...
          RequestURL.sureDividInfos(params).then(res=>{
            ...
            this.dealQueryLog(this.dealId,this.dealId,'6','411','确认分成')
          }).catch(err=>{
            this.confirmDivideBtn = false
            this.$message({
              type: 'error',
      ...
      dealBreach(){
        this.$confirm('确认要提交违约吗？','警告',{
          ...
          let params = {dealId: this.params.dealId}
          RequestURL.dealBreach(params).then(res=>{
            ...
            this._getTableData()
            this.dealQueryLog(this.dealId,this.dealId,'6','412','违约')
            this.$emit('submit',1)
          }).catch(err=>{
            this.dealBreachBtn = false
            this.$message({
      ...
      cancelDealBreach(){
        this.$confirm('确认要取消违约吗？','警告',{
          ...
          RequestURL.cancelDealBreach(params).then(res=>{
            ...
            this._getTableData()
            this.dealQueryLog(this.dealId,this.dealId,'6','413','取消违约')
            this.$emit('submit',0)
          }).catch(err=>{
            this.cancelDealBreachBtn = false
            this.$message({
      ...
      _getTableData(){
        let params = {id: this.params.dealId,dealType: '00'}
      ...
      //查询过户流程信息
      queryTransferSchedulePage(){
        RequestURL.queryTransferSchedulePage({dealId : this.params.dealId}).then(res=>{
          this.transitData = res
        }).catch(err=>{
          console.log(err)
        })
      },
    ...
    computed: {
      caculateActive(){
        for(let i in this.transitData){
          if(this.transitData && this.transitData[i].status === '1'){
            let ii = (Number(i)) + 1
            this.active = ii
          }
        }

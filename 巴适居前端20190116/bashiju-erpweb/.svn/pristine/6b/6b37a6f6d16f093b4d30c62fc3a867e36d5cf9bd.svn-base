      ...
      handleAddHouseClick (action, _params) {
        if (action === 1) {
          ...
          addHouse({dataJson: JSON.stringify(params)}).then(res => {
            ...
            // 向后台传递日志数据
            let message = {
              sourceCode: _params.titles, // 资源标识
              sourceTypeId: _params.transactionTypeId, // 资源类型
              operatTypeId: this.$DICT_CODE.LOG.BUSINESS_OPERATE_TYPE.ADD_HOUSE, // 操作类型
              logContent: `新增二手房源：${_params.titles}` // 日志内容
            }
            this.$updateLog.house.houseAddLog({message: JSON.stringify(message)})

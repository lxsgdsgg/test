<template>
  <div class="page-content">
      ...
      <div class="query-form" ref="query-form">
        <el-form size="small" :inline="true" :model="queryForm" ref="queryForm" class="demo-form-inline">
          ...
          <el-form-item prop="trusteesId">
              <base-cascader
                placeholder="经办人" clearable
                v-model="queryForm.trusteesId"
                :url="getReferenceUserSelectUrl"
               :props="cascaderProps"
                :dataProps="cascaderDataProps"
                @change="handleTrustees"
                style="width: 130px"
    ...
    <template>
      ...
      >
        <receipt-template
          @printOver='printEndHandler' :code="printData.code" :payMoney="printData.payMoney" :payMan="printData.payMan"
      ...
      >
        <div>
          <ContractContent v-if="hackResetContract"
             @canelContract="cancelContract"
             @nextOperation="contractNextOperation"
             :houseId="dealParams.houseId"
             :houseCode="dealParams.houseCode"
             :templateType="dealParams.templateType"
             :companyId="dealParams.houseCompanyId"
             :houseCompanyName="dealParams.houseCompanyName"
             :deptId="dealParams.houseMainterDeptId"
             :deptName="dealParams.houseMainterDept"
             :maintainId="dealParams.houseMaintainId"
             :maintainer="dealParams.houseMaintainer"
             :demandId="dealParams.demandId"
             :customerName="dealParams.customerName"
             :custCompanyId="dealParams.custCompanyId"
             :custCompanyName="dealParams.custCompanyName"
             :custMaintainDeptId="dealParams.custMaintainDeptId"
             :custMaintainDeptName="dealParams.custMaintainDeptName"
             :custMaintainId="dealParams.custMaintainId"
             :custMaintainName="dealParams.custMaintainName"
          ></ContractContent>
        </div>
      ...
      name:'depositManage',
        data() {
            return {
              ...
              dealParams:{},//成交参数
              houseIdTemp:'',//房源Id临时接收值
              houseCodeTemp:'',//房源编号临时接收值
              templateType:'',
        ...
        methods: {
          handleTrustees(value,name,data){
            if(data.dataType !== 'user'){
              setTimeout(() => {
                  this.queryForm.trusteesId = ''
              }, 50)
              return false
            }
          },
          ...
          handleContract(row){
            ...
            if(row.transactionTypeId === 5){
              ...
              this.templateType = '02'
            }else{
              this.dialogContractTitle = '买卖合同'
              this.templateType = '01'
            }
            this.dealParams = {
                houseId:row.shhId,
                houseCode:row.houseId,
                companyId:row.houseCompanyId,
                houseCompanyName:row.houseCompanyName,
                deptName:row.houseMainterDept ,
                deptId:row.houseMainterDeptId,
                templateType:this.templateType,
                maintainId:row.houseMaintainId,
                maintainer:row.houseMaintainer,
                demandId:row.demandId,
                customerName:row.custName,
                custCompanyId:row.custCompanyId,
                custCompanyName:row.custCompanyName,
                custMaintainDeptId:row.custMainterDeptId,
                custMaintainDeptName:row.custMainterDept,
                custMaintainId:row.custMaintainId,
                custMaintainName:row.custMaintainer,
            }
          ...
          handlerPrint(row){
            ...
            this.$nextTick(()=>{
              ...
              this.printData.payMoney = row.price/100

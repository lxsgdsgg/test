<template>
  <div class="page-content">
    ...
    <template>
      ...
      <b-dialog :show.sync="dialogAddRate" title="设置提成比例" width="650px">
         <el-button type="primary" size="small" style="margin-top:10px;margin-bottom:10px;margin-left:80%;" @click="handleConfirmAddRate">新增</el-button>
         <el-table
              :data="rateData"
              border
              highlight-current-row
            ...
            >
              ...
              <el-table-column
                align="center"
                label="金额(元）" width=140px>
                <template slot-scope="scope">
                  <template v-if="scope.row.edit">
                    <el-input class="edit-input" :readonly="scope.row.isReadonly" size="mini" @blur="blurMaxPrice(scope.$index,scope.row)"
                              v-model="scope.row.minPrice"></el-input>
                  </template>
              ...
              >
                <template slot-scope="scope">
                  ...
                  <el-button size="mini" plain icon="el-icon-delete" v-if="scope.row.minPrice > 0 || scope.row.minPrice===''"
                             @click="handleDeleteRate(scope.$index, scope.row)"></el-button>
                </template>
              </el-table-column>

            </el-table>
            <el-button type="primary" :loading="loadingSaveRate" style="margin-top:20px;margin-left:45%" @click="handleConfirmEditRate('',false)">保存</el-button>
            <el-button @click="dialogAddRate=false">取消</el-button>
      </b-dialog>
      <b-dialog :show.sync="dialogAddActualrate" title="设置实收比例" width="650px">
        <el-button type="primary" style="margin-top:10px;margin-bottom:10px;margin-left:80%;" size="small" @click="handleConfirmAddActualRate">新增</el-button>
         <el-table
              :data="actualrateData"
              border
              align="center"
              style="width: 100%"
              height="380" size="mini"
            >
              <el-table-column
                prop="sign"
                align="center"
            ...
            <el-button type="primary" :loading="loadingSaveActualRate" style="margin-top:20px;margin-left:45%" @click="handleConfirmEditActualRate" size="mini">保存</el-button>
            <el-button @click="dialogAddActualrate=false" size="mini">取消</el-button>
      </b-dialog>
      ...
      <b-dialog :show.sync="dialogAddUniversalRate" :title="universalRateTitle" width="360px">
        <el-form v-loading="loadingForm" :model="addUniversalRateForm"  ref="addUniversalRateForm" label-width="100px">
          ...
          <el-form-item class="margin-b-none">
            <el-button type="primary" :loading="loadingSaveUniversalRateBtn" @click="saveUniversalRate" size="mini">确认</el-button>
            <el-button @click="dialogAddUniversalRate=false" size="mini">取消</el-button>
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      handleDeleteRate(index,row){
          ...
          if(cminPrice!=''){
            this.handleConfirmEditRate(delId,true);
          }
      },
      ...
      handleDeleteActualRate(index,row){
        ...
        this.delActualData.cminactualRate = row.cminactualRate
        this.delActualData.commissionsRate = row.commissionsRate
        this.actualrateData.splice(index,1)
        if(row.cminactualRate !='' ){
     ...
     //保存通提比例
      ...
      //保存提成比例
      handleConfirmEditRate(delId,isDel){
        ...
        for (let i = 0; i < this.rateData.length; i++) {
          if(i === 0 && this.rateData[i].commissionsRate == ''){
            canSave = false
          }
          else if (((i != 0) && (this.rateData[i].minPrice == '' || this.rateData[i].commissionsRate == ''))) {
            canSave = false
          } else {
            let dataItem = {

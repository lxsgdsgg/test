<template>
     <div>
          <el-form :model="form" ref="form" :rules="rules" label-width="125px" class="demo-form">
            <el-row>
              <el-col :span="12">
        ...
        methods: {
          setForm(data){
              for(let i in this.form){
                  ...
                  this.form.trusteesId = String(this.data.trusteesId)//转类型
              }
            this.form.maybeDealDate = this.$utils.timeFormat(data.maybeDealDate,'{y}-{m}-{d}')
            this.form.entrustTime = this.$utils.timeFormat(data.entrustTime,'{y}-{m}-{d}')
            this.form.maybeDealPrice = this.form.maybeDealPrice/1000000
                    ...
                    if(res){
                      this.$confirm('确定要修改定金信息吗？',{
                        ...
                        type:'warning'
                          ...
                          params.maybeDealPrice =  params.maybeDealPrice*1000000
                          ...
                          updateDepositData(params,cfg).then(res =>{
                            if(res){
                              ...
                              let message = {
                                ...
                                sourceTypeId: this.data.transactionTypeId,
                                operatTypeId: 216,
                              ...
                              console.log(message)
                              RequeryLogURL.customerUpdateLog({message:JSON.stringify(message)}).then(res =>{
                                console.log(res)
                              }).catch(error =>{
                                console.log(error)
                              })

...
<template>
  <div style="min-height: 550px">
    <el-button v-if="btnPermission[PERMISSION_BTN.ADD_PRESALE]" @click="handleAdd" type="primary" plain icon="el-icon-circle-plus" size="small">新增</el-button>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button v-if="btnPermission[PERMISSION_BTN.UPDATE_PRESALE]" @click="handleEdit(scope.row)" icon="el-icon-edit" size="mini">编辑</el-button>
          <el-button v-if="btnPermission[PERMISSION_BTN.DEL_PRESALE]" @click="handleDelete(scope.row)" icon="el-icon-delete" size="mini">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      ...
      <el-form ref="form" :model="form" :rules="rules" size="medium" label-width="100px">
        <el-form-item label="许可证号" prop="name">
...
<script>
  ...
  import {queryPresalePermitListInfoById, saveOrUpdatePresalePermitListInfo, delPresalePermitListInfo} from '@/request/house/houseNew'
    ...
    methods: {
        ...
        this.$refs['form'].validate(valid=> {
          if (valid) {
            this.$confirm('确定保存编辑的信息?',
              ...
              saveOrUpdatePresalePermitListInfo({jsonData: JSON.stringify(params)}).then(() => {
                this.$message({
                  type: 'success',
                  message: '操作成功',
                  showClose: true,
                ...
                if (this.currentRowData) {
                  ...
                  message = Object.assign({}, {
                    logContent: `许可证号：${this.form.name}, 发证日期：${this.form.issuingDate}`,
                    operatTypeId: 321
                  }, message)
      ...
      handleDelete (row) {
        this.$confirm('确定删除该条数据吗?', {
          ...
          delPresalePermitListInfo({id: row.id}).then(res => {
            this.$message({
              type: 'success',
              message: res.msg
            })
            ...
            let message = {
              ...
              businessTypeId: this.$DICT_CODE.LOG.BUSINESS_TYPE.NEW_HOUSE, // 业务类型
              sourceTypeId:this.$DICT_CODE.LOG.BUSINESS_SOURCE_TYPE.NEW_DEAL,//资源类型
              operatTypeId: 323, // 操作类型
              logContent: `删除预售证号 => ${row.name}`
            }
      ...
      _getTableData () {
        ...
        queryPresalePermitListInfoById(params).then(res => {

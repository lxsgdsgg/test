        ...
        <el-tab-pane label="基本信息" name="dealInfo">
          <div>
            ...
            <div class="pull-right" style="margin-top: 1px;">
              <el-button v-hasMultipleBtn="['updateDealInfo',dealInfo]" type="primary" size="mini"
                         @click="handlerModify" v-if="dealInfo.isBreach !== 1">修改成交</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      handlerReset(isBreach){
        this.dealInfo.isBreach = isBreach
      },
      // 成交保存
      saveDeal (result) {
        if (result) {

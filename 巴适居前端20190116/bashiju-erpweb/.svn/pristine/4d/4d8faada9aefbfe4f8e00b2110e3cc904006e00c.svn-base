      ...
      <div class="query-form" ref="query-form">
        <el-form size="small" :inline="true" :model="queryForm" ref="queryForm" class="demo-form-inline">
          ...
          <el-form-item prop="maintainId">
            <base-cascader
               placeholder="维护人"
               clearable v-model="queryForm.maintainId"
               :url="getReferenceUserSelectUrl"
               :props="cascaderProps"
               :dataProps="cascaderDataProps"
               style="width: 130px"
               :changeOnSelect="false"
               @change="handleMaintainer"
            >
            </base-cascader>
          </el-form-item>
  ...
  export default {
      ...
      mixins: [PageList],
        ...
        methods: {
          ...
          //跟进人
          handleFollower(value,name,data){
            if(data.dataType !== 'user'){
              setTimeout(() => {
                this.queryForm.followerId = ''
              }, 50)
            }
          },
          //维护人
          handleMaintainer(value,name,data){
            if(data.dataType !== 'user'){
              setTimeout(() => {
                this.queryForm.maintainId = ''
              }, 50)
            }
          }
        },

...
public interface IHouseService {
	...
	/**
	 * 
		 * 按区域搜索租房房源列表，房源默认条件isPriority=1,isSynchron=1,statusId=1,examineStatus=1,isValid=1
		 * @Description: 按区域搜索租房房源列表
		 * @param param 
		 * @param page 当前页
		 * @param limit 每页条数
		 * @return 
		 * WebPage<HouseListResult>
	 */
	WebPage<HouseListResult> queryRentHouseListByArea(RentHouseParam param,int page,int limit);
	
	/**
	 * 
		 * 按地铁搜索租房房源列表，房源默认条件isPriority=1,isSynchron=1,statusId=1,examineStatus=1,isValid=1
		 * @Description: 按地铁搜索租房房源列表
		 * @param param 
		 * @param page 当前页
		 * @param limit 每页条数
		 * @return 
		 * WebPage<HouseListResult>
	 */
	WebPage<HouseListResult> queryRentHouseListByMetro(RentHouseParam param,int page,int limit);
	/**
	 * 
		 * 房源比较查询
		 * @Description: 房源比较查询

...
public class PrivacyProtectionBaseServiceImpl implements PrivacyProtectionBaseServiceApi{
    ...
    public BindAxnExtensionResponse bindAxnExtension(String Phone){
        ...
        IClientProfile profile = DefaultProfile.getProfile("cn-hangzhou", SYSGlobal.plsaccessKeyId,  SYSGlobal.plsaccessKeySecret);
        try {
        ...
        //必填:对应的号池Key
        request.setPoolKey(SYSGlobal.poolKey);
        //必填:AXN关系中的A号码
        ...
        if(response.getCode()==null || !response.getCode().equals("OK")) {
        	logger.error("阿里云原因导致隐私号码未绑定成功,错误码原因为{}",response.getMessage());
        	return null;
        }
        return response;
    }
    ...
    public boolean unbind(String subsId, String secretNo) {
	    ...
	    if(response.getCode()==null && !response.getCode().equals("OK")) {
	    	logger.error("阿里云原因导致隐私号码解绑未成功，错误码原因为{}",response.getMessage());
	    	return false;
	    }
	    return true;
    }

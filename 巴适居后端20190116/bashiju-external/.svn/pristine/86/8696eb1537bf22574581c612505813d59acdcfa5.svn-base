...
public class LogFinConsumerMq {
	...
	public void auxAddLog(BusinessLogModel businessLogModel) {
		...
		try {
			...
			logger.warn("【财务新增】操作日志传入的对象businessLogModel，值是：{}",jsonString);
			...
			logger.error("【财务】消息队列添加【新增操作】日志出错,值是{}，原因是：{}",jsonString,e.getMessage());
	...
	public void auxUpdateLog(BusinessLogModel businessLogModel) {
		...
		try {
			routKey=businessLogModel.getRoutKey();
			jsonString=JSON.toJSONString(businessLogModel);
			logger.warn("【财务更新】操作日志传入的对象businessLogModel，值是：{}",jsonString);
			Map<String,Object> map=JSON.parseObject(jsonString,Map.class);
			...
			logger.error("【财务】消息队列添加【更新操作】日志出错,值是{}，原因是：{}",jsonString,e.getMessage());
			rabbitTemplate.convertAndSend("ERP-EXT-DELAY-EXCHANGE",routKey, businessLogModel);
		}
	}
	...
	public void auxDeleteLog(BusinessLogModel businessLogModel) {
		...
		try {
			routKey=businessLogModel.getRoutKey();
			jsonString=JSON.toJSONString(businessLogModel);
			logger.warn("【财务删除】操作日志传入的对象businessLogModel，值是：{}",jsonString);
			Map<String,Object> map=JSON.parseObject(jsonString,Map.class);
				if (map.get("originalData")!=null) {
					map.remove("originalData");
				}
			...
			logger.error("【财务】消息队列添加【删除操作】日志出错,值是{}，原因是：{}",jsonString,e.getMessage());
			rabbitTemplate.convertAndSend("ERP-EXT-DELAY-EXCHANGE",routKey, businessLogModel);
		}
	}
	...
	public void auxQueryLog(BusinessLogModel businessLogModel) {
		...
		try {
			routKey=businessLogModel.getRoutKey();
			jsonString=JSON.toJSONString(businessLogModel);
			logger.warn("【财务查询】操作日志传入的对象businessLogModel，值是：{}",jsonString);
			Map<String,Object> map=JSON.parseObject(jsonString,Map.class);
				if (map.get("originalData")!=null) {
					map.remove("originalData");
				}
			...
			logger.error("【财务】消息队列添加【查询操作】日志出错,值是{}，原因是：{}",jsonString,e.getMessage());
			rabbitTemplate.convertAndSend("ERP-EXT-DELAY-EXCHANGE",routKey, businessLogModel);
		}
	}
	/**
	...
	private void insertData(Map<String,Object> map) {
		...
		try {
			...
			logger.error("【财务】消息队列日志【添加到数据库】出错,值是{}，原因是：{}",map,e.getMessage());
		}
	}
	...
	/*private void insertReport(BusinessLogModel model) {
		...
		if (map.get("operatTypeId")!=null) {
			...
			try {
				...
				logger.error("【财务】消息队列日志【添加到数据表report_operation_analysis】出错,值是{}，原因是：{}",map,e.getMessage());
			}
		}
	}*/

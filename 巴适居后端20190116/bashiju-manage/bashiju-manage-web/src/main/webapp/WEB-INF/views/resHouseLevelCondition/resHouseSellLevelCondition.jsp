...
<div class="layui-container">
	<form class="layui-form" action="" id="menuForm" style="margin-top: 20px;" name="res_house">
		<input type="text" name="transactionType" value="1" autocomplete="off" class="layui-input" style="width:400px;display: none ">
	  ...
	  <div class="layui-form-item">
	   ...
	   <div class="layui-form-item">
		    ...
		    <div class="layui-input-inline">
			    <select  id="houseUses"></select>
		    </div>
		     <label class="layui-form-label">付款方式</label>
		    <div class="layui-input-inline">
			    <select name="payTypeId" id="payTypeId">
			    		<option value="">请选择</option>
			    		<option value="5">只能一次性付款</option>
			    		<option value="6">可商业贷款</option>
			    		<option value="7">可公积金(租客)贷款</option>
			    		<option value="8">可分期付款</option>
			    </select>
		    </div>
 ...
 layui.use(['form','formSelects'], function(){
	...
	//条件  
	  ...
	  $.post(path+"/manage/getSelectValue/many",{param:"res_house"},function(d){
		...
		form.render('select');
		formSelects.value('select', "${res_houses.conditions}".split(",")); //初始化条件多选框
	});
	//获取区间数据值
		  ...
		  form.on('submit(res_housesForm)', function(data){
		    ...
		    if(conditions!="")
		     ...
		     if(price!="")	
    	 	 ...
    	 	 if(payTypeId!="")	
		    	 data.field.remark=data.field.remark+"<font style='color:black; font-weight:bold'>付款方式:</font>"+payTypeName+";"; 
		    $.ajax({
	    	type:"post",

...
layui.use(['table','layer','form'], function(){
  ...
  table.render({
    ...
    elem: '#dynamicform'
    ,height: 'full-200'
    ,url: '/manage/dynamicConfigure/queryCompanyDynamicFormList' //数据接口
    ,page: true //开启分页
    ,cols: [[ //表头
...
//监听工具条
  table.on('tool(dynamicform)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    if(layEvent === 'fieldDetail'){
    	...
    	showAddField(data.tableChinaName,data.tableDbName,data.customTableName,data.id);
    } 
  });
 ...
 function showFieldTable(formId){
	 table.render({
	    ...
	    ,height: 'full-200'
	    ,url: '/manage/dynamicConfigure/queryCompanyDynamicFormFieldList' //数据接口
	    ,where:{formId:formId}
	    ,page: true //开启分页
	    ,cols: [[ //表头
	    ...
	    if(layEvent === 'updateField'){
	    	...
	    	layer.confirm('您确定要删除字段【'+data.title+'】吗?', {icon: 3, title:'确认'}, function(index){
	    		 $.ajax({
	    	         ...
	    	         data: {id:data.id},
	    	         url: "<%=request.getContextPath()%>/manage/dynamicConfigure/delCompanyFormField" ,//url	                
	    	         //data:data,
 ...
 function showAddField(cName,tName,customTableName,formId){
	 $.ajax({
         type: "POST",//方法类型
         dataType: "json",//预期服务器返回的数据类型
         //contentType: 'application/json',
         data: {formId:formId},
         url: "<%=request.getContextPath()%>/manage/dynamicConfigure/queryCompanyDynamicFormFieldSelect" ,//url	                
         //data:data,
         success: function (result) {
         	if(result.success){
         		setAddFieldForm(cName,tName,customTableName,formId,result.data);
         	}else{
         		layer.alert(result.msg);
         	}
         },
 ...
 function setAddFieldForm(cName,tName,customTableName,formId,selectData){
	 ...
	 $("#addFieldForm input[name=customTableName]").val(customTableName);
	 ...
	 form.on('submit(saveFieldBtn)', function(data){
		 $.ajax({
	            ...
	            url: "<%=request.getContextPath()%>/manage/dynamicConfigure/saveCompanyFormField" ,	                
	            //data:data,
	            success: function (result) {
	            	if(result.success){
	            		layer.close(layerId);

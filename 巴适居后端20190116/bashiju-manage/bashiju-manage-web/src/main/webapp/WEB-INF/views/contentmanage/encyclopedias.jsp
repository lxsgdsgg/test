...
<html>
	...
	<body class="">
	    ...
	    layui.use(['element','form','table','layer','laydate','upload'], function(){
			  ...
			  var conditions = {type:"encyclopedias"};
			  table.render({
			    elem: '#articleList'
			    ,height: 500
			...
			for(var key in articleClassList){
				...
				if(cityCode==key){
					for(var i=0;i<d.length;i++ ){
						if(d[i].level==1){
							var ppdiv = $('<div style="font-size:16px;" class="pptype" id="'+d[i].id+'">'+d[i].name
							         +'</div>' )
							$(".type-box").append(ppdiv);
						}else if(d[i].level==2){
							var pdiv = $('<div class="ptype" style="margin-left:10px;" id="'+d[i].id+'" >'+d[i].name
							         +'</div>' )
							$("#"+d[i].parentId).append(pdiv);
						}else{
							var cdiv = $('<div class="ctype" style="margin-left:10px;" id="'+d[i].id+'" onclick="selectOneClass(this);">'+d[i].name
							         +'</div>' )

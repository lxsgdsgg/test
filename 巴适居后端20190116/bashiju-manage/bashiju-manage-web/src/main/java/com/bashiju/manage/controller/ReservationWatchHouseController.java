...
//		 页面转换
	...
	@ResponseBody
	public Map<String,Object> queryReservationWatchHouseByAgentId(String reservationDate,String status,int page,int limit){
		Page<Map<String,Object>> pages =  reservationWatchHouseService.queryReservationWatchHouseByAgenId(reservationDate,status,page, limit);
	...
	@ResponseBody
	public BashijuResult updateReservationTime(String jsonData) {
		if(StringUtils.isEmpty(jsonData))
			throw new BusinessException("保存参数有误");
		Map<String,Object> paramMap = (Map<String,Object>) JSONObject.parse(jsonData);
		paramMap.put("handelerId",UserThreadLocal.get().get("id"));
		paramMap.put("status",2);
		paramMap.put("operator",UserThreadLocal.get().get("realName"));
		paramMap.put("operatorId",UserThreadLocal.get().get("id"));
		paramMap.put("handelTime",new Date());
		paramMap.put("handelResult","已取消");
		boolean result = reservationWatchHouseService.updateReservationsInfo(paramMap);
		if(result)
			return BashijuResult.ok();
		throw new BusinessException(ErrorCodeEnum.SYSTEM_UPFDATE_ERROR);
	...
	/***
	 * 
	 * @Title: acceptlReservation   
	 * @Description: 接收预约
	 * @param jsonData
	 * @return: BashijuResult
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(value="acceptlReservation")
	@ResponseBody
	public BashijuResult acceptlReservation(String jsonData) {
		if(StringUtils.isEmpty(jsonData))
			throw new BusinessException("保存参数有误");
		Map<String,Object> paramMap = (Map<String,Object>) JSONObject.parse(jsonData);
		paramMap.put("handelerId",UserThreadLocal.get().get("id"));
		...
		paramMap.put("operatorId",UserThreadLocal.get().get("id"));
		paramMap.put("handelTime",new Date());
		paramMap.put("handelResult","已接受预约");
		boolean result = reservationWatchHouseService.updateReservationsInfo(paramMap);
		if(result)
			return BashijuResult.ok();
		throw new BusinessException(ErrorCodeEnum.SYSTEM_UPFDATE_ERROR);

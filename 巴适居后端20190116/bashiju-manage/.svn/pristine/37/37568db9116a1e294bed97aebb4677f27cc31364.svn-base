...
import com.bashiju.enums.LogSystemOperateTypeEnum;
...
public class CommunityServiceImpl implements ICommunityService {
	...
	public Page<Map<String, Object>> queryCommunityList(CommunityQueryEntity query,int page,int limit) throws Exception {
		...
		if (query.getRegionId()!=null) {
			map.put("regionId", query.getRegionId());
		}
	...
	public String addCommunity(String name,String recordName,String pinyin, String areaCode, Long regionId, String address, int price, int isHot) {
		...
		communityMapper.communitySynchron(id.toString());
	...
	@Override
	public long updateCommunity(Map<String,Object> map) {
		...
		communityMapper.communitySynchron(param.get("id").toString());
		Map<String, Object> project = communityMapper.queryProjectId(Long.parseLong(param.get("id").toString()));
		if (project!=null&&project.get("id")!=null) {
			communityMapper.newHouseSynchron(project.get("id").toString());
		}
		return result;
	}
		...
		if (multipartResolver.isMultipart(request)) {
			...
			if (iter.hasNext()) {
				...
				if (file != null) {
					...
					Map<String, Object> project = communityMapper.queryProjectId(Long.parseLong(communityId+""));
					if (project!=null&&project.get("id")!=null) {
						communityMapper.newHouseSynchron(project.get("id").toString());
					}
					//记日志
					SystemLogModel model=new SystemLogModel();
					model.setSourceCode(communityName);
					model.setSourceTypeId(Integer.parseInt(LogSystemSourceTypeEnum.COMMUNITY.getCode()));
					model.setOperatTypeId(Integer.parseInt(LogSystemOperateTypeEnum.UPLOADPIC.getCode()));
					model.setPath(filepath);
					model.setLogContent(communityName+"小区上传图片");
					systemLogService.systemPlatformLogAdd(model);
	...
	public Long updateImage(Map<String,Object> data){
		...
		if(res1) {
			res = 1l;
			Map<String, Object> project = communityMapper.queryProjectId(Long.parseLong(data.get("communityId").toString()));
			if (project!=null&&project.get("id")!=null) {
				communityMapper.newHouseSynchron(project.get("id").toString());
			}
		}else {
	...
	public boolean deleteImage(String ids) {
		if (StringUtils.isNotEmpty(ids)) {
			...
			if (param.size()>0) {
				...
				Long cid = Long.parseLong(paths.get(0).get("communityId").toString());
				commonSqlServie.delData("sys_community_attachment", "id", ids, false);
				communityMapper.communitySynchron(cid.toString());
				Map<String, Object> project = communityMapper.queryProjectId(cid);
				if (project!=null&&project.get("id")!=null) {
					communityMapper.newHouseSynchron(project.get("id").toString());
				}
			}

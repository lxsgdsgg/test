...
public class CityOpenInfoServiceImpl extends CommonSqlServie implements CityOpenInfoService {
	...
	public boolean saveCityOpen(Map<String,Object> map) {
		...
		if(!map.containsKey("id") || StringUtils.isEmpty(map.get("id").toString())) {//新增
			//验证该城市平台信息是否已经存在
			...
			this.commonOperationDatabase(map, "sys_cityOpenInfo","id", false);
		}
		
		ExecutionResult.descFormat("", "保存城市开通");
		return true;
	}
	 ...
	 * @param list 待保存的信息
	 * @return: boolean
	 * @see com.bashiju.manage.service.CityOpenInfoService#saveNavigationBarPermission(java.lang.String, java.util.List)
	 */
	@Override
	@SystemServiceLog(operationType="保存城市授权导航")
	public boolean saveNavigationBarPermission(String cityOpenInfoId,List<Map<String, Object>> list) {
		//应先将之前的信息删除
		cityOpenInfoMapper.deleteNavigationBarPermission(cityOpenInfoId);
		boolean result = this.batchCommonOperationDatabase(list, "sys_cityOpen_function", false);
		ExecutionResult.descFormat(cityOpenInfoId, "保存城市授权导航");
		return result;
	}

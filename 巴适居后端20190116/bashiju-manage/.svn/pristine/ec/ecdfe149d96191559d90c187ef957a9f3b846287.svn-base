...
<div class="layui-container">
	<div class="layui-form">
		<div class="layui-form-item">
			<label class="layui-form-label">待授权城市</label>
    		<div class="layui-input-block">
			<c:forEach items="${responsibility}" var="responsibility" varStatus="index">
				<input type="radio" lay-skin="primary" lay-filter="responsibilityCity" ${index.count==1?"checked":null} name="responsibilityCity"  title="${responsibility.cityName}" value="${responsibility.cityCode}"/>
			</c:forEach>
			</div>
		</div>
	</div>
...
layui.use(['form','element','table', 'laydate','tree'], function(){
	...
	//监听提交
	  form.on('submit(permissionForm)', function(data){
		...
		var responsibilityCity = $("input[name='responsibilityCity']:checked").val();
		if(responsibilityCity=="" || responsibilityCity==undefined){
			layer.alert("请选择要授权的城市");
			return false;
		}
		if(roleId==""){
	    ...
	    $.ajax({
	    	...
	    	dataType:"json",
	    	data:{responsibilityCity:responsibilityCity,data:JSON.stringify(data.field),roleId:roleId,userId:userId},
	    	success:function(dt){
	    		if(dt.success){
	  ...
	  form.on('radio(responsibilityCity)', function(data){
		  getPermissions(data.value);
		});
	...
	function getPermissions(permission){
		...
		$.ajax({
			...
			url:"getPermissions",
			dataType:"json",
			data:{permission:permission,roleId:roleId,userId:userId},
	...
	$(document).ready(function(){
		var responsibilityCity = $("input[name='responsibilityCity']:checked").val();
		getPermissions(responsibilityCity);
// 		var per = ${per};

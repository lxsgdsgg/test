...
public class PerformanceTypeServiceImpl extends CommonSqlServie implements IPerformanceTypeService {
	...
	public Page<Map<String, Object>> queryPfmTypeList(Map<String, Object> paraMap
		...
		dataAuthHelper.auth(MenuEnum.MENU_770.getCode()
	...
	public void saveOrUpdatePfmTypeData(Map<String,Object> paraMap) {
		long pfmTypeId = 0L;
	...
	public void saveOrEditPfmTypeDetail(List<Map<String,Object>> paraList,String companyId) {
		...
		List<Map<String, Object>>  tempList = 
				pfmTypeMapper.queryPfmTypeDetailByCondition(null,companyId);
		//如果此公司在该城市已经存在佣金配置，则先删除再保存
		if(tempList.size() > 0) {
			delData(ManageGlobal.PERFORMANCE_TYPE_CONFIG_DETAIL,"companyId" ,"'" + companyId + "'", false);
			ExecutionResult.descFormat(companyId, "删除佣金配置");
		}
		batchCommonOperationDatabase(paraList ,ManageGlobal.PERFORMANCE_TYPE_CONFIG_DETAIL,false);
	...
	public List<Map<String,Object>> queryPfmTypeDetailByCondition(Map<String,Object> paraMap){
		...
		List<Map<String, Object>>  tempList = 
				pfmTypeMapper.queryPfmTypeDetailByCondition(rgCode,companyId);
		return tempList;
	}
}

...
public class ResHouseReturnCycleServiceImpl extends CommonSqlServie implements ResHouseReturnCycleService {
	...
	public boolean saveOrUpdateResHouseReturnCycle(List<Map<String, Object>> map) {
		...
		if(map.get(0).containsKey("id")) {
			...
			result = this.batchCommonOperationDatabase(map, "res_house_return_cycle", "id", false);
			if(result) {
			...
			result =this.batchCommonOperationDatabase(map, "res_house_return_cycle", false);
			if(result) {
				ExecutionResult.descFormat(String.valueOf(result),"批量添加房源回访周期");
				return true;
			}else
				return false;
		}

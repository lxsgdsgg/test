...
@Service
 public class FormServiceImpl implements FormService{
	...
	@SystemServiceLog(operationType="判断表单自定义列是否有值")
	@Override
	public boolean queryCountCustomValue(String keyId, String formName) {
		int count=formmapper.queryCountCustomValue(keyId, formName);
		ExecutionResult.descFormat(keyId+","+formName, "判断表单自定义列是否有值");
		return count>0;
	}

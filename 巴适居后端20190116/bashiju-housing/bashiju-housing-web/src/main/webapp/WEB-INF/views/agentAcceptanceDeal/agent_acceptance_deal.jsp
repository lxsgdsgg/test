...
<body>
	...
	<form id="detailForm" class="layui-form layui-hide" action="" style="margin-top:20px">
		<div id="detailInfo" class="layui-tab-item layui-show">
	    	...
	    	<div class="layui-col-md3">
	    		<div class="layui-input-inline">
		    		<button type="button" onClick="newHouseAdd()" class="layui-btn">新增房源</button>	
		    	</div>
	    	</div>
	    	<hr>
	...
	<script type="text/javascript">
		var regionInfo = JSON.parse('${communitList}');
		var communnityInfo = JSON.parse('${communitList}');
		...
		var community = new MultiSelection(communnityInfo, function(code, name,
				level) {
			if (level == 3) {
		...
		//新增房源
		function newHouseAdd(){
			$('#addForm').removeClass("hidden");
		  	$("#addForm input[name=houseUsesId]").val('house');
			$("#addForm input[name=houseUses]").val('住宅');
			newlayerid = layer.open({
				type: 1
			  	,title: "新增房源" //不显示标题
			  	,area:['550px','450px']
			  	,content: $('#addForm')

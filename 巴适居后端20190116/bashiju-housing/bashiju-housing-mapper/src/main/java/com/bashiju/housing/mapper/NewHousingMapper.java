...
public interface NewHousingMapper {
	 ...
	 * 
		 ...
		 * Map<String,Object>
	 */
	Map<String,Object> queryHouseDealInfoByHouseId(@Param("houseId")Long houseId);
	/**
	 * 
		 * 查询房源采集列表
	 ...
	 * 
		 * 查询用户姓名、手机号
		 * @Description: 查询用户姓名、手机号
		 * @param userId 用户id
		 * @return 
		 * Map<String,Object>
	 */
	public Map<String,Object> queryUserPhone(@Param("userId")Long userId);
	/**
	 * 
		 * 根据楼盘id、户型图面积索引下标，查询crc32编码
		 * @Description: 根据楼盘id、户型图面积索引下标，查询crc32编码
		 * @param projectId 楼盘id
		 * @param spaceIndex 户型图面积索引下标
		 * @return 
		 * String
	 */
	public String querySpaceQueryParam(@Param("projectId")Integer projectId,@Param("spaceIndex")String spaceIndex);
}

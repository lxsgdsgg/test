	...
	<form id="customData" class="form-horizontal" style="margin:20px 0;">
		<div>
			<ul class="custom-inline">
			 <c:if test='${! empty customField}'>
				 <c:forEach var="item" items="${customField}">
						<li>
							...
							${item.title}</label>
						      <input  type="text" name="${item.field}"   class="form-control pull-left customColumn" value="" style="width:100px;" >
						</li>
				 </c:forEach>
			 </c:if>
			</ul>
...
if('${customValue}'!=''){
	var customValue=JSON.parse('${customValue}');//自定义字段的值
}else{
	var customValue = '';
}
...
function showNextForm(){
	 ...
	 // $('#demandData select[name=transactionTypeId]').val(customerInfo.transactionTypeId);

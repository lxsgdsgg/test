...
import com.bashiju.anal.mapper.StatisticsRentDemandMapper;
...
public class StatisticsRentDemandController {
	
	@Autowired
	private StatisticsRentDemandMapper statisticsRentDemandMapper;
	 ...
	 * @return: ModelAndView
	 */
	@RequestMapping(value="statisticsRentDemand")
	...
	public ModelAndView getReportDetailInfo(HttpServletRequest request,HttpServletResponse response) {
		...
		if(DemandStatisticsLongitudinalParamsEnum.L1.getCode().equals(longitudinal)) {//状态
			...
			transverse_list = statisticsRentDemandMapper.queryCityArea(companyId, cityCode);
...
//			cell_property = " b.buildSpace ";
			...
			transverse_list = statisticsRentDemandMapper.queryResourceType();
		}else if(DemandStatisticsLongitudinalParamsEnum.L5.getCode().equals(longitudinal)) {//价格区间
//			cell_property = " b.sellingPrice ";
			transverse_list = statisticsRentDemandMapper.queryCityavgpricelabel(cityCode);
...
//					String min = map.get("min");
		...
		List<Map<String,Object>> dataList = statisticsRentDemandMapper.getData(transverse,longitudinal,companyId,cityCode,cell_sql.toString(),deptId,

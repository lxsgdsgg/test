...
import com.bashiju.webservice.mapper.IFormMapper;
...
public class FormServiceImpl implements IFormService{
	 ...
	 */
	@Override
	public Map<String,List<FormRangeResult>> queryRangeByCode(String codes,String cityCode){
		Map<String,List<FormRangeResult>> resMap = new HashMap<>();
		String key = WebGlobal.RANGE_ITEM_REDIS_PREFIX+cityCode;
		String rangeStr = cacheService.getCache(key);
		List<FormRangeResult> rangeList = null;
		if(StringUtils.isEmpty(rangeStr)) {
			...
			rangeList = JSON.parseArray(rangeStr, FormRangeResult.class);
		}
		String[] codearr = codes.split(",");
		for(int i=0;i<codearr.length;i++) {
			List<FormRangeResult> resList = new ArrayList<>();
			for(FormRangeResult r : rangeList) {
	...
	public Map<String,List<SelectItemResult>> querySelectByCode(String codes){
		...
		String key = WebGlobal.SELECT_ITEM_REDIS_KEY+codes;

...
public class CustomerPersonalCentralServiceImpl implements ICustomerPersonalCentralService{
	...
	public CustCustomerEntity queryPersonalInfo(String custId,String numType) {
		...
		return mCustomerPersonalCentralMapper.queryPersonalInfo(custId,numType);
	}
	/**
	...
	public CustCustomerEntity queryPersonalDetailInfo(String custId,String numType) {
		CustCustomerEntity custEntity = mCustomerPersonalCentralMapper.queryPersonalDetailInfo(custId,numType);
		...
		long concerndCount = mICustomerConcernedHouseService.countMyConcerned(custId);
		...
		custEntity.setConcernedCount(concerndCount);
	...
	public void modifyCustomerPassword(String custId,String oriPassword, String finalPassword) {
		CustCustomerEntity user = this.queryPersonalInfo(custId,"00");//00代表普通账户类型，由于密码存在普通账户表，所以固定
		if(null == user) {
...
//		}
	...
	public boolean deleteAppointment(String id) {
		mCustomerPersonalCentralMapper.deleteAppointment(id);
		return true;
	}
}

...
public class CommunityServiceImpl implements CommunityService {
	...
	public List<HouseListResult> querySealHousingByCommunityId(String communityId,Integer room) {
		if(StringUtils.isEmpty(communityId))
			throw new BusinessException("小区编号不能为空");
		String key = Md5EncryptUtils.md5Encrypt("querySealHousingByCommunityId"+communityId+room);
		String jsonData = cacheService.getCache(key);
		if(!StringUtils.isEmpty(jsonData)) {
			List<HouseListResult> list = JSONArray.parseArray(jsonData, HouseListResult.class);
			return list;
		}else {
			List<HouseListResult> selllHousing = communityMapper.querySealHousingByCommunityId(communityId,room);
	...
	public List<HouseListResult> queryLeaseHousingByCommunityId(String communityId,Integer room) {
		 if(StringUtils.isEmpty(communityId))
			throw new BusinessException("小区编号不能为空");
		 	String key = Md5EncryptUtils.md5Encrypt("queryLeaseHousingByCommunityId"+communityId+room);
			String jsonData = cacheService.getCache(key);
			if(!StringUtils.isEmpty(jsonData)) {
				List<HouseListResult> list = JSONArray.parseArray(jsonData, HouseListResult.class);
				return list;
			}else {
				List<HouseListResult> leaseHousing = communityMapper.queryLeaseHousingByCommunityId(communityId,room);

...
public class CustomerEntrustmentServiceImpl implements ICustomerEntrustmentService{
	...
	public void saveHouseEntrustmentData(CustomerHouseEntrustmentEntity houseEntity
		...
		if(agentInfo.size() == 0) {
			log.warn("未找到该小区负责的经纪人");
			throw new BusinessException(ErrorCodeEnum.NULL_OBJ.getCode());
		}
		...
		try {
			long id = mCustomerEntrustmentsMapper.saveHouseEntrustmentData(saveMap);
			...
			throw new BusinessException(ErrorCodeEnum.SYSTEM_ADD_ERROR.getCode());
		}
	}
	/**
	 * 校验客户输入验证码是否正确

 ...
 */
public class MapHouseSellPageServiceApiImpl implements IMapHouseSellPageServiceApi {
	@Autowired
	...
	public List<QueryCondition> queryConditionsSellHouse(String cityCode) {
		List<QueryCondition> results=commService.queryQueryConditions(cityCode, ConditionTypeEnum.SELLHOUSE);
		List<QueryCondition> list= new ArrayList<>();
		if (results!=null&&results.size()>0) {
			for (QueryCondition queryCondition : results) {
				if (!queryCondition.getConditionCode().equals("labels")&&!queryCondition.getConditionCode().equals("dianti")) {

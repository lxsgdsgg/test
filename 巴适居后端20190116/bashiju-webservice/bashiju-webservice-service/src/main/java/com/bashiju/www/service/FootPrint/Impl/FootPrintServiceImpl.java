...
public class FootPrintServiceImpl extends CustomerPersonalCentralBaseServiceApi implements FootPrintService{
	...
	public boolean saveFootPrint(FootPrintToSaveEntity footPrintToSaveEntity) {
		if(StringUtils.isEmpty(footPrintToSaveEntity.getKey()))
			throw new BusinessException("用户登录信息不能为空");
		if(StringUtils.isEmpty(footPrintToSaveEntity.getType()))
			throw new BusinessException("足迹类型不能为空");
		if(StringUtils.isEmpty(footPrintToSaveEntity.getSourceId()))
		...
		long isExist = footPrintMapper.queryIsExist(custId, footPrintToSaveEntity.getType(), footPrintToSaveEntity.getSourceId());
		if(isExist>0) {
			return false;
		}

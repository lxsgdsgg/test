...
import com.bashiju.webservice.mapper.CustomerTransitScheduleMapper;
...
public class CustomerHouseTransitScheduleServiceImpl 
	...
	public WebPage<CustomerHouseTransitScheduleEntity> queryHouseTransitScheculeDataList(
		...
		if(retList.getPageSize() == 0) {
			...
			for(CustomerHouseTransitScheduleEntity obj : pageObj) {
				obj.setScheduleEntity(mCustomerTransitScheduleMapper.queryTransitScheculeDataList(obj.getDealSequence()));
				obj.setTransitHouseEntity(mCustomerTransitScheduleMapper.queryTransitHouseByHouseId(obj.getShhid()));
			}
			if(pageObj.size() > 0) {
	...
	public List<CustomerTransactionScheduleEntity> queryTransitScheculeDataList(String dealId){
		return mCustomerTransitScheduleMapper.queryTransitScheculeDataList(dealId);
	}

...
public class ContractServiceImpl implements ContractService {
	...
	public Page<Map<String, Object>> queryContractList(int page, int limit) {
		...
		dataAuthHelper.auth(MenuEnum.MENU_75.getCode(),UserThreadLocal.get().get("id").toString());
				...
				if (items!=null&&items.size()>0) {
					...
					for (int j = 0; j < items.size(); j++) {
						if (items.get(j).get("termId").toString().equals(terms.get(i).get("id").toString())) {
							...
							if (items.get(j).get("tag").toString().equals("date")||items.get(j).get("tag").toString().equals("beginTime")
									||items.get(j).get("tag").toString().equals("endTime")) {
								item.put("year", "");
								item.put("month", "");
								item.put("date", "");
							}
	...
	@Override
	public Long addTemplate(String templateName, String type,String codePrefix, String remark) {
	...
	@Override
	public int updateTemplate(Long id, String templateName, String type,String codePrefix, String remark) {
		Map<String,Object> param=new HashMap<>();
		param.put("templateName", templateName);
		param.put("remark", remark);
		param.put("templateType", type);
	...
	public Long saveContract(String dataJson) throws IOException {
		if (StringUtils.isNotEmpty(dataJson)) {
			...
			if (data.get("commissionA")!=null&&!"".equals(data.get("commissionA").toString())) {
				data.put("commissionA", Integer.parseInt(data.get("commissionA").toString()));
				
			}else {
				data.put("commissionA", 0);
			}
			if (data.get("commissionB")!=null&&!"".equals(data.get("commissionB").toString())) {
				data.put("commissionB", Integer.parseInt(data.get("commissionB").toString()));				
			}else {
				data.put("commissionB", 0);
			}
			...
			Map<String, Object> signMan=contractMapper.queryUserInfo(Integer.parseInt(data.get("signPerson").toString()));
			data.put("signPerson",signMan.get("id"));
			data.put("signPersonName",signMan.get("realName"));
			data.put("status", "00");
			data.put("permissionArea", signMan.get("deptId"));
			data.put("operatorId", signMan.get("id"));
			data.put("signCompanyId", signMan.get("companyId"));
			data.put("signDeptId", signMan.get("deptId"));
			data.put("signDeptName", signMan.get("deptName"));
			data.put("signCompanyName", signMan.get("companyName"));
			data.put("isCross", 0);
			if (!data.get("houseCompanyId").toString().equals(data.get("custCompanyId").toString())) {
				data.put("isCross", 1);
			}
	...
	public List<Map<String, Object>> queryContractListByType(String templateType) {
		...
		List<Map<String, Object>> list=contractMapper.queryContractListByType(templateType,UserThreadLocal.get().get("companyId").toString());
		return list;
	}
	...
	@Override
	public Map<String, Object> queryContractByCode(String code) {
		Map<String, Object> result=contractMapper.queryContractByCode(code);
		return result;
	}

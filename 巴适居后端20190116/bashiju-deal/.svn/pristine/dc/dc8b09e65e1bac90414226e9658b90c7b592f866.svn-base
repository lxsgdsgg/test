...
layui.use(['table','laydate','form'], function(){
  var table = layui.table;
    ...
    layui.laytpl={
			...
			toIsDivideStr : function(isDivide){
				if(isDivide==0)
					return '未分成'
				else
					return '已分成'
  ...
  table.render({
    ...
    ,height:'full-100'
    ,url: 'queryDealFinanceDividenInfoData'
   	,method:'post'
    ,page: true //开启分页
    ,cols: [[ //表头
      {field: 'address', title: '地址', sort: true, fixed: 'left'}
      ,{field: 'dealId', title: '成交编号', sort: true}
      ,{field: 'addTime', title: '收取日期', sort: true,templet: '<div>{{ common.toDateString(d.addTime,"yyyy-MM-dd HH:mm:ss") }}</div>'}
      ...
      ,{field: 'isDivide', title: '分成状态',templet: '<div>{{ layui.laytpl.toIsDivideStr(d.isDivide) }}</div>'}
